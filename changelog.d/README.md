...
无法判断时按“使用者升级后是否需要知道”裁决。选择“不需要碎片”的 MR 必须在 MR 模板中写明理由。
...
- 详细碎片默认使用中文，公共类型、命令、字段和稳定 code 使用反引号保留原名；
- 定版时还必须在根 CHANGELOG 的目标版本段写一份由
  `<!-- PUB_CHANGELOG:START -->` / `<!-- PUB_CHANGELOG:END -->` 包围的英文摘要；
  四包 CHANGELOG 只从该摘要派生，以满足 pub.dev 的主要语言要求；
