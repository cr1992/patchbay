...
void main() {
  group('ReleaseFinalizer (PB-041-02)', () {
    ...
    test(
      'generates plan and blocks when unverified items exist without allowDefer',
      () {
        ...
        File(
          '${tempDir.path}/docs/releases/0.4.1.md',
        ).writeAsStringSync('> 状态：规划中\n');
...
''');
        ...
        expect(
          ...
          anyElement(contains('PB-041-02 is "实现中"')),
        );
        expect(
          plan.items.firstWhere((i) => i.id == 'PB-041-01').action,
          'ARCHIVE',
        );
        expect(
          plan.items.firstWhere((i) => i.id == 'PB-041-02').action,
          'BLOCK',
        );
      },
    );
...
''');
      ...
      final success = finalizer.applyPlan(repoRoot: tempDir.path, plan: plan);
      expect(success, isTrue);

      final backlog = File(
        '${tempDir.path}/docs/backlog.md',
      ).readAsStringSync();
