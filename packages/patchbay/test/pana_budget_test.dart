...
void main() {
  group('PanaBudgetEvaluator (PB-041-01)', () {
    ...
    test('validates full score for compliant package structures', () {
      ...
      final pkgDir = Directory('${tempDir.path}/packages/test_pkg')..createSync(recursive: true);
...
''');
    ...
    test('penalizes missing LICENSE, relative links in README, and missing repo', () {
      final tempDir = Directory.systemTemp.createTempSync('pana-test-');
      addTearDown(() => tempDir.deleteSync(recursive: true));

      final pkgDir = Directory('${tempDir.path}/packages/bad_pkg')..createSync(recursive: true);
      // No LICENSE
      File('${pkgDir.path}/README.md').writeAsStringSync(
...
''');
      ...
      expect(check.issues, anyElement(contains('Missing or empty description')));
...
''';
      final failedCheck = evaluator.parsePubScoreResponse('patchbay', penaltyJson);
