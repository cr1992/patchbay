...
///
/// The four package manifests are the version source of truth. The root README
...
final RegExp _readmeStatusVersion = RegExp(r'\*\*项目状态：\*\*\s*`v([^`]+)`');
...
List<String> checkReleaseVersionParity({
  ...
  final Map<String, String?> readmeVersions = <String, String?>{
    '项目状态': _firstCapture(_readmeStatusVersion, readme),
...
''';
  ...
  test('真仓文件：四包 version 与 README 状态及安装 refs 一致', () {
    ...
    final List<String> problems = checkReleaseVersionParity(
      pubspecs: pubspecs,
      readme: File(_readmePath).readAsStringSync(),
    );

    expect(
      problems,
      isEmpty,
      reason: 'pubspec.yaml 是版本真源；README 的状态与可复制 Git ref 必须随发版同改',
    );
  });
}
