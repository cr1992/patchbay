...
`patchbay_transport` 是 consumer-neutral 的纯 Dart 调试传输。它用固定 HTTP/JSON 协议承载
`identity`、`catalog`、`snapshot` 和 `invoke`，不依赖 Flutter、Moii、plugin、VM Service 或 CLI。
...
experimental，并仅用于受信任、隔离的同一网络；不能称为 secure。需要跨不受信网络时，应在后续产品层
...
Android、iOS 与 HarmonyOS 都只消费本包的 `dart:io` socket；本批没有 native 改动：

- Android：产品层后续选择并声明网络权限，只在允许的调试构建装配；本包不改 manifest；
- iOS：LAN 模式是否申请 Local Network 权限是产品装配层的后续选择，本包不改 Info.plist、不触发权限
