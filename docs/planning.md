...
必须改为“已接受”，并把长期设计结论同步到 `design.md`。把 Proposal 推进为“已接受”的 MR 必须在描述中
指出 `design.md` 的同步段落；若全部结论仅对单版本有效而无需同步，也必须逐条说明理由，不能只勾选完成。
...
上面几条判的是"同一时刻各文档是否自洽"。下面五条判的是**时间语义**——同一份台账在版本推进中是否
仍然成立。它们此前只写成文字规则，靠人在评审里发现，于是 0.4.0 发布后有八条 PB 一直挂着
`target: 0.4.0` / `实现中`，跨了两个版本没人看见。

- **活跃版本由计划状态派生，不再是工具里的手写常量。** 非 `已发布` 的版本计划必须恰好一份，那一份
  就是活跃版本；零份是版本间歇期的合法状态，此时不做范围对账。两份及以上直接判红——上一版没发布就
  开下一版，"范围真源是哪一份"没有答案。取消常量是因为漂移的成因就在"切换"这个动作上：常量、范围表
  和碎片 `target` 是三处独立写入，任何一处忘改，机检都看不出来。
- **已发布版本上不得残留非终态 target。** `target` 指向已发布版本的条目必须处于终态：PB 条目是
  `已验证` 或 `待真机验收`，`DG` 条目是 `已裁决`（两类各按自己的状态词表判，不共用一套）。真延期就按
  延期流程把 `target` 清成 `—`，不允许让条目继续声称目标是一个已经发出去的版本。
- **`target` 必须解析到真实存在的版本计划。** 指向没有计划文件的版本号是范围漂移：那个版本既没有
  优先级表也没有退出条件。
- **版本启动顺序。** 活跃计划必须以规范形态声明**唯一一行** `> 版本分支：\`dev/<SemVer>\``，并在同一行
  写明从稳定 `main` 创建；活跃版本号必须严格大于每个已发布版本。
- **活跃计划不得夹带起草期瞬时事实。** 诸如「本分支 base 上……」「并行分支已登记……」这类描述"写这份
  计划的那一刻某个分支上有什么"的句子，几小时后就可能不成立；提交 SHA 也只允许出现在版本分支声明行。
  已发布计划是版本史，不受这条约束。

这五条**不覆盖**的边界，写在这里而不是留给下一个人重新发现：

- 不验证版本分支声明里的 base SHA 确实是 `main` 的祖先——CI 常为浅克隆，拿不到 `main` 历史。它拦的是
  "声明缺失或声明了别的起点"，不是"确实从别处拉的但照抄了正确声明"。
- SHA 判据要求 token 同时含 `a-f` 字母与数字，因此全字母的短 SHA（`deadbee`）会漏过。这是卫生检查而
  不是安全边界：漏一个不改变结论，误把日期或计数判成 SHA 却要人来查，所以宁可漏。
- 版本号比较只看三段数字，预发布后缀不参与。仓内约定是 RC 用**状态**表达而不是改版本串，因此
  `docs/releases/` 下不出现 `X.Y.Z-rc.N` 这类文件名；一旦出现，比较会把它与正式版视为相等。
- 版本分支声明行整行豁免 SHA 检查，且「从稳定 `main`」是子串匹配。该行按设计是版本身份，不防对抗性书写。
- 零份未发布计划是合法的版本间歇期，此时不做范围对账；`docs/releases/` 被清空也落在这一档，由
  `target` 解析检查兜底。
