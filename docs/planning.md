...
- **已发布版本上不得残留非终态 target。** `target` 指向已发布版本的 PB 条目，状态只允许 `已验证` 或
  `待真机验收`。真延期就按延期流程把 `target` 清成 `—`，不允许让条目继续声称目标是一个已经发出去的版本。
- **`target` 必须解析到真实存在的版本计划。** 指向没有计划文件的版本号是范围漂移：那个版本既没有
  优先级表也没有退出条件。
- **版本启动顺序。** 活跃计划必须以规范形态声明 `> 版本分支：\`dev/<SemVer>\``，并写明从稳定 `main`
  创建；活跃版本号必须严格大于每个已发布版本。**边界要说清**：机检不验证声明里的 base SHA 确实是
  `main` 的祖先（CI 常为浅克隆，拿不到 `main` 历史），它拦的是"声明缺失或声明了别的起点"。
