...
| 0.6.0 | [接入漏斗记录](0.6.0-onboarding-funnel.md) | 无先验 Agent 已量测三次，三次分别对应三个候选 SHA：997s → 651s → **198s**；第三次（`1279c581`，含全部已知文档与脚本修复）六步一次通过、零 `error` 信封，**达成 10 分钟目标**，安全写操作经 capture 出的 PNG 独立核对确已落到 UI | 只证明 Agent 一侧；**无先验用户一行仍是空的**，PB-060-05 的验收条件（用户 + Agent 各一次）因此仍未满足，10 分钟结论不外推到人。三行 SHA 不同，只可比趋势不可当重复观测；单次量测不构成分布。第三次受试者检出移除了 `CLAUDE.md`/`AGENTS.md` 以防宿主自动注入联调姿势，与前两行存在方法学差异 |
