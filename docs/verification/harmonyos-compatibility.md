...
> 状态：阻塞于可构建 SDK 基线、接入方应用与 HarmonyOS 真机
>
> 记录日期：2026-08-18
>
...
Hvigor SDK 完整性检查处失败，且没有可见的 HarmonyOS target。接入方应用未进入本次工作区，也没有进行
安装、启动、授权或系统弹窗操作。
...
- `patchbay_flutter` example 只用于 package 导入、依赖解析和 HAP 构建预检；它没有接入方应用的路由、
  业务 adapter、权限声明和生命周期接线。
- 真机验收对象统一为 **真实接入方应用**。VM attach、identity/catalog/invoke、Semantics、lifecycle、artifact、
  App 权限 API、UiTest/Hypium 与 hdc 恢复都必须在接入方应用的固定 revision 上取证。

最终将 `supportStatus` 改为 `verified` 前，六步矩阵必须全部是 `verified`，且每步至少有一条可复核证据；
example 成功只能解除构建前置阻塞，不能把任何接入方应用项标绿。
...
| 真机验收 App | 接入方应用 | 本工作区不可用，未执行 |

“候选”不等于项目已经选定 SDK。要固定基线，必须先让同一个 SDK root 通过 HAP 构建，并把 SDK
distribution、revision、API level 与接入方应用 revision 一起固定进 fixture；不能只因为 `flutter doctor`
能看到工具链就升级结论。
...
|---|---|---|---|---|
| 1. core + Flutter build | example 预检；最终为接入方应用 | `patchbay`/`patchbay_flutter` 从固定源码解析，接入方应用 debug/profile HAP 用选定 SDK 构建成功 | `blockedBySdk` | Hvigor `00303168`；接入方应用未进入验收 |
| 2. VM Service attach | 接入方应用真机 | attach 后 identity、catalog、invoke 均成功，requestId/admission 可核对 | `blockedByDevice` | 无 HarmonyOS target、无接入方 HAP |
| 3. Semantics/lifecycle/artifact | 接入方应用真机 | identifier 可读；前后台状态准确；artifact 可分块读取且预算生效 | `blockedByDevice` | 无 HarmonyOS target、无接入方 HAP |
| 4. App permission API | 接入方应用真机 | camera/location 状态可读，请求可触发；原始平台状态与 factSource 有证据 | `blockedByDevice` | 无 HarmonyOS target、无接入方权限 adapter |
| 5. UiTest/Hypium | 接入方应用真机 | runner 只处理预声明 camera/location 窗口；permission/decision 不匹配 fail-closed | `blockedBySdk` | runner 未构建，SDK 与设备均未固定 |
| 6. hdc recovery | 接入方应用真机 | hdc 断连、App 前后台、一次性授权回收后重新握手、刷新 catalog、重解析目标 | `blockedByDevice` | 无 HarmonyOS target、无接入方 HAP |
...
以下动作只在用户提供选定 SDK、HarmonyOS 真机和接入方应用固定 revision 后执行：

**证据落点：** 接入方应用名称、applicationId、包名、设备 model/serial 和 revision 属于接入方信息，
按 [`AGENTS.md`](../../AGENTS.md) 的保密规则不进版本库。这类取证材料写入被 gitignore 的
`.local/verification/`；本报告与 fixture 只保留中性结论、阻塞原因和 capability 状态。

1. 记录 SDK distribution/revision、SDK API/build、设备与接入方应用的 build mode，更新 baseline；
   识别性字段留在 `.local/`，不记录签名材料或设备凭据；
2. 先构建接入方应用 debug/profile HAP；失败时保留构建错误，矩阵不继续伪绿；
3. 通过 hdc 启动接入方应用，仅对该 applicationId 建立 VM Service attach，验证
   identity/catalog/invoke；
4. 读取一个固定 Semantics identifier，执行前后台切换并读取 lifecycle，再下载一个无敏感内容的
   artifact；
5. 通过接入方应用权限 adapter 查询并请求 camera/location；UiTest/Hypium runner 只在预声明窗口身份
...
- 一份 Hvigor 能完整识别、可离线构建接入方应用的选定 OpenHarmony/HarmonyOS SDK；
- `ohpm`/`hvigorw` 的明确发现路径（不要求修改全局 PATH，可以由验收脚本显式提供）；
- 一台 `hdc list targets` 可见的 HarmonyOS 真机；
- 接入方应用的固定 revision、debug/profile HAP、applicationId 与权限 adapter/runner 接线。
