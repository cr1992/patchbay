...
> 状态：已接受
>
> 关联：PB-040-12、PB-040-14；YAML manifest 输入与 `--emit-manifest` 生成初稿已随 0.4.0 发布，见
...
- 恢复失败不覆盖原始巡检失败的退出码断言。
- 双平台 example 预检各验证至少三屏及一个非常驻控件；失败注入覆盖导航拒绝和恢复失败。真实 App 的屏数与
  遮挡由接入方补充（发布判据见[版本计划 SC-040-01](../../releases/0.4.0.md#范围变更记录)）。
