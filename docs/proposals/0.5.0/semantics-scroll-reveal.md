...
- policy **问在容器 + 具体 scroll action 上**：目标未挂载时没有目标可问是硬事实；副产物是 reveal
  不扩权（今天拒绝 scroll action 的接入方自动拒绝 reveal），采纳。
- `maxSteps` **默认 40 / 硬上限 200 采纳**：数值随本 Proposal 冻结，实现 MR 不得悄调。
- 目标未挂载且多滚动容器时 **确定拒绝并指引 `--container`**：「从最内层试起」是全树启发式的开端，
  与非目标红线的思想相悖；确定性拒绝 + 显式参数符合封闭词表立场。
- example 懒加载屏 **与 PB-050-22 合批**为同一个接入面 MR，避免两个 MR 改同一 example 文件，
  并落实小件攒批的交付纪律。
