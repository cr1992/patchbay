...
- 不改 compact 编码器；`$.notice` 通用删除**采纳**（仅作用于 accepted 路径，拒绝路径不走投影门、
  `rejection.notice` 恒保留，可解释性不受影响）；catalog `commands[].summary` **保留**（describe 之前
  的唯一线索）；不为本条新设 DG。
- 与 PB-050-20 的接缝顺序冻结：先落盘、后投影，brief 不得删除 artifact 指针，也不登记已不在文档
  里的删除。
