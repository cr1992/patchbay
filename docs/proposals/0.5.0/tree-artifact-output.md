...
- 默认阈值 65536 字节、保留策略 7 天 / 128 MiB、`--max-inline-bytes` 0=关闭内联：均采纳本稿建议值。
- 不为本条新设 DG：阈值内逐字节不变，默认行为未破。
- 新增 error code 随实现 MR 同一提交进 PB-050-23 封闭注册表。
- 与 PB-050-21 的接缝顺序冻结：**先落盘（本条）→ 后投影（brief）**，brief 不得删除 artifact 指针。
