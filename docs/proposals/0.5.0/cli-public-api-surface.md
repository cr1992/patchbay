...
### 裁决结论（DG-050-07，2026-08-26，仓主授权代理裁决）

仓主在会话中授权代理裁决，并提供两个内部接入方仓库执行了中性 import 扫描（授权与过程记录于
本 MR；扫描在接入方仓库本地完成，结论以中性口径记录，不引入任何业务标识）：

- **接入方 A**：源码零 `package:patchbay_cli` import，依赖仅为可执行调用。**2 + 8 收口对其
  零影响。**
- **接入方 B**：恰一处脚本（及其测试）import canonical library，实际使用
  `PatchbaySessionStore` / `PatchbaySessionRecord`（含 `write` / `remove`）：以自有方式启动
  App（不适用 `patchbay launch` 的受监护路径），取得服务 URI 后直接写 pending 会话记录以获得
  免 `--ws-uri` 的自动发现，退出时清理记录。需求真实且与业务无关。

裁决：

1. **接受 2 + 8 封闭清单，不因扫描扩表。** 上述需求真实且通用，但把 session store/record 冻进
   SDK 是错误的表达方式：会话记录是内部演进面——PB-050-14（已接受）即将为记录追加 workspace
   identity 字段，SDK 直写方无法计算这些字段，只能永远产出 legacy 记录并失去亲和性保障。冻结
   内部格式为公共 API 与本 Proposal 的收口方向正对冲。
2. **认定被删除符号承载的真实需求为「注册 / 注销一个外部启动的 App 会话」，迁移路径 =
   新增小型本地 CLI 命令**（`session register --ws-uri <uri> ...` 与对应注销），JSON 边界、复用
   既有 pending record 语义与字段、不新增记录字段；从接入方 checkout 内执行时自动获得
   PB-050-14 的 workspace 亲和性——严格优于 SDK 直写现状。登记为 backlog 新条目
   PB-050-27，与 PB-050-13 同批交付：13 的「接入方换 pin 后编译门禁通过」依赖该命令先可用，
