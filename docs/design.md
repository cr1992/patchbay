...
`patchbay.dart` 是 114 符号的默认 consumer façade；`patchbay_host.dart` 在它之上追加 38 个 host
lifecycle 符号（共 152）；`patchbay_protocol.dart` 是 150 个 raw wire 与 protocol 符号，与前两者有意
重叠。Flutter 默认入口只在 core consumer 上追加 `PatchbayKey`、`PatchbayRoot`、
`PatchbayRootController`、`PatchbayUiRegistry`（共 118），service host、bridge 与 policy 进入显式
`patchbay_flutter_host.dart`（211）。公共 barrel 必须用封闭 `show`，API checker 按 library 展开跨包
