...
`patchbay.dart` 是 114 符号的默认 consumer façade；`patchbay_host.dart` 在它之上追加 43 个 host
lifecycle 符号（共 157）；`patchbay_protocol.dart` 是 150 个 raw wire 与 protocol 符号，与前两者有意
...
`patchbay_flutter_host.dart`（216）。公共 barrel 必须用封闭 `show`，API checker 按 library 展开跨包
