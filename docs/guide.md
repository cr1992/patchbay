...
| `package:patchbay/patchbay_host.dart` | host implementer | 157 | 默认清单的严格超集，再加 `PatchbayServiceHost`、audit、invocation/cancellation 与 validation lifecycle |
...
| `package:patchbay_flutter/patchbay_flutter_host.dart` | Flutter 组合根 | 216 | core host 清单加全部 Flutter service host、bridge、policy、inspector、navigation、gesture、semantics、reveal 与 capture 符号 |
...
**预算是两层的，别把它们当成一个。** `--max-steps`（1..200）与 `--timeout-ms`（1..120000）是
**调用级硬顶**，在解析容器之前就求值，因此越界即拒，与这次调用最终有没有驱动任何容器无关。接入方注入
的 `revealPolicy` 是**容器级**授权——它回答的是"允不允许驱动这个容器、以及这个容器的预算是多少"，
只在已经选出将被驱动的容器之后才被问到。于是有两条路径上 policy 根本不会被求值：目标**已经露出**
（无需驱动，返回 `steps: 0` 成功）和**没有可驱动的滚动容器**（返回容器类拒绝）。这两条路径上，一个
越过 policy 上限但仍在 1..200 内的 `--max-steps` **不会**换来 `uiRevealBudgetExceeded`——那里不存在
