...
| PB-050-14 | workspace / worktree 级会话亲和性 | session 记录虽保存 `workspacePath`，默认 resolver 仍从全局目录按显式 ID、全局 pin、唯一性选择；在多个 checkout 并行时，旧 pin 可把无 `--session` 的 Agent 命令带到另一工作区的 App | 0.5.0 | 已排期 | [Workspace 会话亲和性](proposals/0.5.0/workspace-session-affinity.md)（已接受）；DG-050-12 已裁决；不引入常驻 daemon 或全局 latest |
