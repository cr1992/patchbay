...
| PB-041-01 | pub.dev 评分可复现门禁：按实际发布 archive 对四包执行当期 Pana，并在发布后核对 score API | 本地 analyze / publish dry-run 全绿仍不能替代 pub.dev 解包后的评分；四包同版依赖链也会让未发布上游造成假扣分 | 0.4.1 | 已验证 | 落地 `tool/check_pana_budget.dart` 与单测，全四包评分预算 140/140 满分且自动校验 README 外链与 metadata |
...
| PB-041-03 | 手写源码结构预算与依赖方向门禁 | 四包生产代码有 10 个手写文件超过 800 行；仅靠评审提醒无法阻止新职责继续堆入热点文件 | 0.4.1 | 已验证 | 落地 `tool/check_structure_ratchet.dart` 门禁与单测，全仓 108 个生产文件全部 ≤ 800 行且零 hand-written part 碎片 |
