...
| PB-050-06 | invocation cooperative cancellation、deadline 与统一受理预算 | direct timeout 后保留 slot 是防止卡死 handler 后继续堆积的刻意语义；registry 路径又未纳入统一运行预算，缺少可证明的取消确认与统一生命周期 | 0.5.0 | 实现中 | [Invocation 生命周期](proposals/0.5.0/invocation-cancellation.md)；DG-050-04 已裁决；execution 默认 8、active-owner record 上限 256；VM/direct 与 legacy/context-aware 降级矩阵已落实现和回归，仓内 example Android 真机预检 95/95 已通过，待 iOS 模拟器与业务接入方验收 |
