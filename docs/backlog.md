...
| PB-041-03 | 手写源码结构预算与依赖方向门禁 | 四包生产代码有 10 个手写文件超过 800 行；仅靠评审提醒无法阻止新职责继续堆入热点文件 | 0.4.1 | 已验证 | 建立 `docs/code-structure.md` 结构规范并落地门禁与单测：四类结构错误判红（含新增的领域环与越包根导入检测），体积改为警戒线；函数长度警戒线（150 行）比文件长度更有信号，实测抓出 14 处、其中 3 处经复核确需拆分 |
