...
| BUG-20260820-02：`ui text set` 对受控表单回执未标记 `drivesState` | `textSet` 仅更新 controller 不触发 `onChanged`，回执却同为 `applied`，导致受控表单状态未更新被误判为按钮点不动；需在回执 payload 中显式返回 `'drivesState': false`（`textEnter` 为 `true`） | 已验证 |
