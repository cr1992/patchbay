...
| PB-040-40 | `patchbay_cli` 源码目录分层与命令分派器模块化 | `patchbay_cli/lib/src/` 扁平放置 28 个文件，`command_registry.dart`（1,926 行）过度集中；按 `commands/`、`platform/`、`session/`、`manifest/` 建立子目录分层 | 0.4.1 | 已排期 | 降低单目录复杂度与代码认知负荷 |
