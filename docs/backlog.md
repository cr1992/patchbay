...
| PB-050-04 | semantics probe 帧放大基准与透明降载 | 每次 probe 都可能主动 `scheduleFrame` 并等待 `endOfFrame`，`ui.wait` 随后还会再等一帧；应先量化帧驱动与遍历各自占比，再只优化有证据的大头 | 0.5.0 | 已排期 | 不预设 identifier 索引为答案；若引入缓存，只存 node 引用并继续以 tree revision 与 generation 复核 |
