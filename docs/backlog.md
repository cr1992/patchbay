...
| PB-050-15 | 锚定式合成 tap（`ui.gesture.tap`） | 手势家族只有 pressHold/drag/fling，最常用的点按缺位；`ui.semantics.tap` 走 performAction 派发，不经指针管线 | 0.5.0 | 实现中 | 裁决见 [0.5.0 版本计划](releases/0.5.0.md)；DG-050-08；[锚定式合成 tap](proposals/0.5.0/anchored-tap.md)；仓内四包与 CLI/widget 矩阵已绿；Android 真机预检（含 tap 可达/App 侧观察/遮挡三节点）与 profile smoke 已跑绿；接入方真机验收随 RC 设备矩阵 |
