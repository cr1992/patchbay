...
>
> 当前排期见 [Patchbay 0.4.1 版本计划](releases/0.4.1.md)。进入版本计划不等于越过设计闸门；
...
| PB-041-01 | pub.dev 评分可复现门禁：按实际发布 archive 对四包执行当期 Pana，并在发布后核对 score API | 本地 analyze / publish dry-run 全绿仍不能替代 pub.dev 解包后的评分；四包同版依赖链也会让未发布上游造成假扣分 | 0.4.1 | 已验证 | 落地 `tool/check_pana_budget.dart` 与单测：实跑当期 pana，四包逐 section 满分（160/160，pana 0.23.18），取不到真实分数即 fail-closed；`--published` 支持发布后核对 score API。发布后 score API 复核仍待执行 |
| PB-041-02 | 发布收尾自动化：按范围变更和证据状态分型归档 backlog，消费目标版本碎片并冻结版本计划 | 0.4.0 发布后仍残留目标行和 changelog 碎片，证明“打完 tag 再人肉清账”不可持续；但实现中与已验证不能直接批量删除 | 0.4.1 | 已验证 | 落地 `tool/release_finalize.dart` 与单测，支持两阶段 plan/apply 归档与未结验证 fail-closed 阻断 |
| PB-041-03 | 手写源码结构预算与依赖方向门禁 | 四包生产代码有 10 个手写文件超过 800 行；仅靠评审提醒无法阻止新职责继续堆入热点文件 | 0.4.1 | 实现中 | 门禁与单测已落地（棘轮基线、领域环检测、tool/ 与 example/ 纳管，141 个生产文件）；但关键验收「新建/完成拆分的生产文件 ≤ 600 行」尚未达成：9 个 0.4.1 拆分产物仍超预算，已在 `tool/structure_baseline.json` 的 accepted_debt 冻结登记，待后续 MR 偿还后方可转「已验证」 |
| PB-041-04 | `patchbay` core host 与领域服务分层 | `service_host.dart` 1,426 行且影响 115 个符号，混合 VM Service 适配、snapshot、invoke、审计、schema 校验；`artifacts.dart` 889 行 | 0.4.1 | 已验证 | 按 `host/` 与 `artifacts/` 完成拆分，原文件收敛为 Facade，全量单测保持 100% 绿灯 |
| PB-041-05 | `patchbay_flutter` host 与 UI bridge 领域分层 | `semantics_bridge.dart` 938 行、`gesture_bridge.dart` 835 行，service host 同时承担 descriptor、decoder 与注册编排，桥接职责持续横向增长 | 0.4.1 | 已验证 | 拆分为 `semantics/` 与 `gesture/` 模块并消除 `part` 碎片，保持 151 个单测绿灯 |
| PB-041-06 | 大型测试套件拆分与共享测试支撑收敛 | 现有 5 个测试文件超过 1,000 行，重复 host/fixture 构造使重构时难以判断行为变化还是测试接线变化 | 0.4.1 | 已验证 | 完成 5 个超千行大型测试套件模块化拆解，提取独立子套件与共享夹具，测试文件全部 ≤ 1000 行且 100% 断言保持绿灯 |
| PB-041-07 | `patchbay_cli` 领域服务继续拆分 | 除 `cli.dart` 外，`command_registry.dart` 1,925 行、`ui_manifest.dart` 1,654 行、`trace.dart` 1,302 行、`doctor.dart` 1,129 行、`session.dart` 830 行，仍会把目录分层变成“移动大文件” | 0.4.1 | 已验证 | 完成 `registry/`、`manifest/`、`trace/`、`doctor/`、`session/` 模块化，全量单测保持 100% 绿灯 |
| PB-041-08 | `release_prep` 定版工具模块化拆解与架构解耦 | `release_prep.dart` 单文件 2,727 行，混合模型、pubspec 扫描、CHANGELOG 碎片聚合、矩阵改写与 IO 编排；拆解为子模块并保持全部测试和 bin 入口兼容 | 0.4.1 | 已验证 | 拆解为 `tool/src/release_prep/` 10 个子模块并保持全部测试与 facade 兼容 |
